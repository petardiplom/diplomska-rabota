    ...
    const sql = `
        ...
        session_count AS (
            SELECT ses.id AS session_id,
              COUNT(CASE WHEN sub.status = 'active' THEN 1 END) AS active_count
            FROM ${Tables.Sessions} ses
              LEFT OUTER JOIN ${Tables.Subscriptions} sub ON ses.id = sub.session_id
            GROUP BY ses.id
        )
        SELECT r.id AS id, r.id AS reservation_id, null AS session_id, 'reservation' AS type, 1 AS active_count,
        s.name AS title, s.color, ss.name AS subservice_name, c.email AS customer_email, su.email AS staff_email,
        r.start_time AS start, r.end_time AS end, r.duration, r.price, r.status::text, r.cancelled_at, r.cancelled_by, 1 AS capacity
        ...
        UNION ALL
        SELECT s.id AS id, null AS reservation_id, s.id AS session_id, 'session' AS type, sc.active_count,
        ser.name AS title, ser.color, ss.name AS subservice_name, null AS customer_email, su.email AS staff_email,
        s.start_time AS start, s.end_time AS end, s.duration, s.price, s.status::text, s.cancelled_at, s.cancelled_by, s.capacity
